...
void process(kvBuffer *kvb){
  ...
  for(kviter = 0 ; kviter < kvb->length ; kviter++){
    ...
    if(kvp->operation == 'R'){
      if(LOAD_FLAG){
	status = db->Put(rocksdb::WriteOptions(),kvp->key,kvp->value);
      }
      else{
	status = db->Get(rocksdb::ReadOptions(),kvp->key, &value);
	read_count++;
      }
      if(!status.ok()){
	cerr<<"key:"<<kvp->key<<" ";
...
void init(char filename[],char dbfilename[],char load_str[]){
  ...
  if(dbfilename == NULL){
    rocksdb::Status status = rocksdb::DB::Open(ops,"testdb",&db);
  }else{
    rocksdb::Status status = rocksdb::DB::Open(ops,dbfilename,&db);
  }
  if(!status.ok()){
...
void consume(){
  ...
  while(true){
    ...
    if(!more_data_to_produce && consume_list.empty()){  
      ...
      std::string stat_str;
      db->GetProperty("rocksdb.stats",&stat_str);
